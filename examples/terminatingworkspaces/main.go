...
import (
	...
	"github.com/kcp-dev/multicluster-provider/terminatingworkspaces"
)
...
func main() {
	var (
		...
		provider          *terminatingworkspaces.Provider
		logLevel          string
	)
	...
	entryLog.Info("Using terminator", "terminator", terminator)

	entryLog.Info("Setting up manager")
	provider, err = terminatingworkspaces.New(cfg, workspaceTypeName, terminatingworkspaces.Options{})
	if err != nil {
	...
	if err := mcbuilder.ControllerManagedBy(mgr).
		Named("kcp-terminator-controller").
		...
		Complete(mcreconcile.Func(
			func(ctx context.Context, req mcreconcile.Request) (ctrl.Result, error) {
				...
				// check if our terminator is still set on the logicalcluster
				if !slices.Contains(lc.Status.Terminators, terminator) {
					logger.Info("Terminator already absent, skipping")
					return reconcile.Result{}, nil
				}

				logger.Info("Starting to terminate cluster")

				// Example cleanup: delete a child workspace that was created during initialization.
				...
				if err != nil {
					if !apierrors.IsNotFound(err) {
						return reconcile.Result{}, fmt.Errorf("failed to check child workspace: %w", err)
					}
					// Child workspace already gone, cleanup is complete.
					logger.Info("Child workspace already deleted", "child", childWorkspace)
				} else {
					// Child workspace still exists, delete it and requeue to wait for deletion.
					logger.Info("Deleting child workspace", "child", childWorkspace)
					if err := client.Delete(ctx, ws); err != nil {
						if !apierrors.IsNotFound(err) {
							return reconcile.Result{}, fmt.Errorf("failed to delete child workspace: %w", err)
						}
					}
					// Requeue to wait for the workspace to be fully deleted.
					return reconcile.Result{RequeueAfter: 1 * time.Second}, nil
				}

				// All cleanup done. Remove the terminator from the LogicalCluster status
				// to unblock workspace deletion. Per kcp docs, use patch (not update).
				patch := ctrlclient.MergeFrom(lc.DeepCopy())
				lc.Status.Terminators = slices.DeleteFunc(lc.Status.Terminators, func(t corev1alpha1.LogicalClusterTerminator) bool {
					return t == terminator
				})
				if err := client.Status().Patch(ctx, lc, patch); err != nil {
					return reconcile.Result{}, fmt.Errorf("failed to remove terminator: %w", err)
				}

				logger.Info("Removed terminator from LogicalCluster status, workspace deletion can proceed")
